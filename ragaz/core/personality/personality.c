...
// Ragaz-specific parts start here!

// Special thanks to Dirkjan Ochtman to their prior work with Runa (https://github.com/djc/runa) project which
// provides these very functional personalty routines

// Keep in sync with Exception class in core/__builtins__.zz
struct RagazException {
...
static inline void
save_caught_exception(struct _Unwind_Exception* ue_header, int handler_switch_value,
                      const unsigned char* lsda, _Unwind_Ptr landing_pad) {
    struct RagazException* exc = (struct RagazException*) ue_header;
...
static inline void
restore_caught_exception(struct _Unwind_Exception* ue_header, int* handler_switch_value,
                         const unsigned char** lsda, _Unwind_Ptr* landing_pad) {
    struct RagazException* exc = (struct RagazException*) ue_header;
...
#define RAGAZ_CLASS 27111979

_Unwind_Reason_Code
__ragaz_personality(int version, _Unwind_Action actions, _Unwind_Exception_Class exception_class,
    ...
    foreign_exception = exception_class != RAGAZ_CLASS;
